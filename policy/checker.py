import re
from policy.articles import articles
from policy.scoring import calculate_score_and_risk, calculate_total_compliance_score
    ...
    def check_compliance(self, text):
        ...
        for article_id, data in self.articles.items():
            ...
            if keywords: # Only perform matching if keywords are defined for the article
                ...
                if matched_count >= 3:
                    status, status_flag, match_percent = "✅ Compliant", "compliant", 100
                elif matched_count == 2:
                    status, status_flag, match_percent = "⚠ Partially Compliant", "partial", 50
                else: # Covers 0 or 1 match
                    status, status_flag, match_percent = "❌ Requires Review", "non-compliant", 15 if matched_count == 1 else 0
        ...
        results["Key Issues"] = [
            f"{idx+1}) {item['name']} ({item['article']}): {emoji.get(item['status'], '')} {item['status'].capitalize()}"
